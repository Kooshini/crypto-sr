...
def scrape_candles_to_csv(filename, exchange_id, max_retries, symbol, timeframe, since, limit):
    ...
    write_to_csv('data/' + filename, ohlcv)
    print('Saved', len(ohlcv), 'candles from', exchange.iso8601(ohlcv[0][0]), 'to', exchange.iso8601(ohlcv[-1][0]), 'to', filename)


# -----------------------------------------------------------------------------

scrape_candles_to_csv('finex-btcusd.csv', 'bitfinex', 3, 'BTC/USD', '1d', '2019-01-01T00:00:00Z', 100)

# -----------------------------------------------------------------------------

df = pd.read_csv('data/finex-btcusd.csv')
# df = df[::-1] #flip if needed
cl = RawPriceClusterLevels(None, merge_percent=0.15, use_maximums=True, bars_for_peak=27)
cl.fit(df)
levels = cl.levels
# print(levels)
